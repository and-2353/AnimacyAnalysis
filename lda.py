...
def cross_validate(): # 実質的にLEAVEONEOUTとして使用
    df = pd.read_csv('nouns/nouns_v10/nouns_v10.2+em.csv', encoding='utf-8')
...
def lda():
    df = pd.read_csv('nouns/nouns_random_label/3208/nouns_random_label(half)+em.csv', encoding='utf-8')
    X = df.drop(['lemma', 'animacy'], axis=1) # 説明変数: embedding
    y = df['animacy'] # 目的変数: 有生性
    words = df['lemma']
